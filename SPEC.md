...
| **Result — partial, second miss** | `FeedbackBanner` (`variant="Partial"`, different copy) + Continue | Tap Continue. | Marks the term retired, then `/session-end` (now populated). **The banner itself was decided to be its own, visually distinct variant** ("visually distinct from Partial... no retry offered") **— it currently reuses the `Partial` variant with different copy instead. Flagged, not yet fixed.** |
